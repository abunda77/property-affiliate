    ...
    <div class="relative h-[70vh] bg-gray-900">
        ...
        @if ($images->count() > 0)
            <div x-data="{
                ...
                <!-- Navigation -->
                <button @click="prev"
                    class="absolute left-4 top-1/2 -translate-y-1/2 bg-white/10 backdrop-blur-md hover:bg-white/20 text-white p-3 rounded-full transition-all opacity-0 group-hover:opacity-100 transform -translate-x-4 group-hover:translate-x-0">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                    </svg>
                </button>
                <button @click="next"
                    class="absolute right-4 top-1/2 -translate-y-1/2 bg-white/10 backdrop-blur-md hover:bg-white/20 text-white p-3 rounded-full transition-all opacity-0 group-hover:opacity-100 transform translate-x-4 group-hover:translate-x-0">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <!-- Thumbnails -->
                <div
                    class="absolute bottom-20 left-1/2 -translate-x-1/2 flex space-x-2 overflow-x-auto max-w-[90vw] p-2 rounded-xl bg-black/20 backdrop-blur-md">
                    <template x-for="(slide, index) in slides" :key="index">
                        <button @click="activeSlide = index"
                            :class="{
                                'ring-2 ring-blue-500 scale-110': activeSlide ===
                                    index,
                                'opacity-70 hover:opacity-100': activeSlide !== index
                            }"
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    @if ($property->specs)
                        @foreach (array_slice((array) $property->specs, 0, 4) as $key => $value)
                            <div
                                class="bg-white p-4 rounded-xl shadow-sm border border-gray-100 text-center hover:shadow-md transition-shadow">
                                <span
                                    class="block text-gray-500 text-xs uppercase tracking-wider mb-1">{{ ucfirst(str_replace('_', ' ', $key)) }}</span>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            @foreach ($property->features as $feature)
                                <div class="flex items-center p-3 bg-gray-50 rounded-lg">
                                    ...
                                    <span
                                        class="text-gray-700 font-medium">{{ is_string($feature) ? $feature : $loop->index }}</span>
                                </div>
                            @endforeach
                        </div>
                    </div>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-3 gap-4" x-data="{ showModal: false, activeImage: '' }">
                            @foreach ($galleryImages as $image)
                                <div class="relative aspect-square rounded-xl overflow-hidden cursor-pointer group"
                                    ...
                                    <img src="{{ $image->getUrl('medium') }}" alt="{{ $property->title }}"
                                    ...
                                    <div
                                        ...
                                        <svg class="w-8 h-8 text-white opacity-0 group-hover:opacity-100 transition-opacity duration-300 transform scale-75 group-hover:scale-100"
                    ...
                    @if ($affiliate)
                        <div class="flex items-center gap-4 mb-6">
                            @if ($affiliate->profile_photo)
                                <img src="{{ Storage::url($affiliate->profile_photo) }}"
                                    class="w-16 h-16 rounded-full object-cover border-2 border-blue-100">
                            @else
                                <div
                                    class="w-16 h-16 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white text-xl font-bold shadow-lg">
                        ...
                        <div class="space-y-3 mb-6">
                            ...
                            <div x-data="{ showBiodata: false }">
                                <button @click="showBiodata = true"
                                    class="group relative flex items-center justify-center w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-semibold py-3.5 px-6 rounded-xl transition-all duration-300 ease-in-out transform hover:scale-[1.02] hover:shadow-xl shadow-lg shadow-blue-500/30 hover:shadow-blue-600/40 overflow-hidden">
                                    <!-- Animated background overlay -->
                                    <div
                                        class="absolute inset-0 bg-gradient-to-r from-white/0 via-white/10 to-white/0 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-700 ease-in-out">
                                    </div>

                                    <!-- Icon with subtle animation -->
                                    <svg class="w-5 h-5 mr-2.5 transition-transform duration-300 group-hover:scale-110"
                                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <!-- Biodata Modal -->
                                <div x-show="showBiodata" style="display: none;"
                                    ...
                                    <div class="absolute inset-0 bg-gray-900/50 backdrop-blur-sm"
                                        @click="showBiodata = false"></div>

                                    <!-- Modal Panel -->
                                    <div class="relative w-full max-w-2xl bg-white rounded-xl shadow-2xl ring-1 ring-gray-200 overflow-hidden"
                                        x-transition:enter="transition ease-out duration-300"
                                        ...
                                        <div
                                            ...
                                            <button @click="showBiodata = false"
                                        ...
                                        <div class="p-6 max-h-[70vh] overflow-y-auto">
                                            <div class="flex items-center gap-4 mb-6">
                                                ...
                                                @else
                                                    <div
                                                        class="w-20 h-20 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white text-2xl font-bold shadow-lg border-4 border-white">
                                                        {{ substr($affiliate->name, 0, 1) }}
                                                    </div>
                                                @endif
                                                <div>
                                                    <h4 class="text-xl font-bold text-gray-900">{{ $affiliate->name }}
                                                    </h4>
                ...
                <div class="bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
                    ...
                    <div class="flex gap-2">
                        <button onclick="navigator.clipboard.writeText(window.location.href)"
                            class="flex-1 bg-gray-50 hover:bg-gray-100 text-gray-600 py-2 rounded-lg transition-colors flex items-center justify-center">
                        ...
                        <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(url()->current()) }}"
                            target="_blank"
                            class="flex-1 bg-blue-50 hover:bg-blue-100 text-blue-600 py-2 rounded-lg transition-colors flex items-center justify-center">
                        ...
                        <a href="https://twitter.com/intent/tweet?url={{ urlencode(url()->current()) }}&text={{ urlencode($property->title) }}"
                            target="_blank"
                            class="flex-1 bg-sky-50 hover:bg-sky-100 text-sky-500 py-2 rounded-lg transition-colors flex items-center justify-center">
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                <path
